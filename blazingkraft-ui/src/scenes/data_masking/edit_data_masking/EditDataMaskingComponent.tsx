...
import { TbAlertCircle, TbPencil, TbX } from 'react-icons/tb';
...
interface EditDataMaskingComponentProps {
    dataMaskingToEdit: IDataMasking;
    isModalOpen: boolean;
    setIsModalOpen: Function;
    editDataMasking: (request: IDataMaskingRequest) => Promise<any>;
...
function EditDataMaskingComponent({
    dataMaskingToEdit,
    setIsModalOpen,
    isModalOpen,
    isEditDataMaskingPending,
    editDataMasking,
}: EditDataMaskingComponentProps) {
    ...
    useEffect(() => {
        if (dataMaskingToEdit) {
            setName(dataMaskingToEdit.name);
            setCode(dataMaskingToEdit.code);
            setDataMaskingType(dataMaskingToEdit.dataMaskingType);
            setRule(dataMaskingToEdit.rule);
            setRuleType(dataMaskingToEdit.ruleType);
            setResult(dataMaskingToEdit.result);
            setTopicType(dataMaskingToEdit.topicType);
            setTopic(dataMaskingToEdit.topic);
        }
    }, [dataMaskingToEdit]);
        ...
        <CommonModal
            ...
            modalTitle={
                <div className="flex items-center break-all">
                    <Text className="pr-2">Edit Data Masking Rule</Text>
                </div>
            }
            onClose={() => setIsModalOpen(false)}
            modalFooter={modalFooter}
            isLoading={isEditDataMaskingPending}
        />
    );
}

export default EditDataMaskingComponent;
